...
# work in progress for poetry encoding with semantic (i.e. <sp>) encoding of speakers
# this script written by Katy Felkner
# advisor: Samuel Huskey

# dictionary of all speakers in the work to make <sp> tags
# in a work with no speakers, this would be empty, and we would never try to make <sp> tags
# options: <head> or <list type="speakers">
speaker_dict = {'M.':"Melioboeus", 'C.':"Corydon", 'A.':"Amyntas"}
    ...
    def lem_wit(wit):
        ...
        if wit == '':
            ...
            for s in split:
                ...
                # TODO: what is the Latin for in margin?
...
# Handle encoding differnt speakers,
# I have tentatively chosen <<>> as delimiters for the speaker.
print('Okay, we\'ll encode all of the speakers first.')
time.sleep(2)
search_speakers = re.compile(r'<<[A-Z]\.>>')
speaker_matches = search_speakers.findall(source_text)
replaceSpeakers = source_text
for m in speaker_matches:
    match_str = m.replace("<", '').replace(">", "")
    sp_tag = "&lt;/sp&gt;&lt;sp who=\"" + speaker_dict.get(match_str) + "\"&gt;&lt;speaker&gt;" + match_str + "&lt;/speaker&gt;"
    replaceSpeakers = replaceSpeakers.replace("<<" + match_str + ">>", sp_tag)

#remove the first, extraneous </sp> tag
search_extra_sp = re.compile(r'&lt;/sp&gt;')
replace_extra_sp = search_extra_sp.sub('', replaceSpeakers, count=1)
...
replace0 = search_addition.sub(r'&lt;supplied reason="lost"&gt;\1&lt;/supplied&gt;', replace_extra_sp)
...
for l in lines:
    if l == '':
        ...
        pass
    elif re.match("&lt;/sp&gt;&lt;sp", l) or re.match("&lt;sp", l):
        # this line starts with a <sp> tag
        print("FOUND A SPEAKER " + l)
        i += 1
        speaker_tag = l.split(" ")[0] + " " + l.split(" ")[1]
        l = speaker_tag + "<l n =\"" + str(i) + "\">" + l.replace(speaker_tag, '') + "</l>"
        print(l)
        newlines.append(l)
    else:
        i += 1
...
replace1 = "".join(newlines)
# add the last </sp> tag
# this is done after <l> tags to avoid wrapping </sp> in an l by itself
replace1 = replace1 + "&lt;/sp&gt;"
print(replace1)
...
replace7 = search_crux.sub(r'&lt;sic&gt;\1&lt;/sic&gt;',replace1)
